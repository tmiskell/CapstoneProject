...
#define MIN_VAL 0                               /* Minimum adjusted flex sensor value. */
#define MAX_VAL 100                             /* Maximum adjusted flex sensor value. */
#define FLEX_BYTES 2
...
int main( int argc, char* argv[] ){
  ...
  for( i = 0 ; i < NUM_FINGERS ; i++ ){
    if( !valid_int(argv[j]) ){
      ...
      lb[i] = MIN_VAL ;
      fprintf( stderr, "*** Adjusted lower bound: %d ***\n", lb[i] ) ;
      j++ ;
    }
    else{
    ...
    if( !valid_int(argv[j]) ){
      ...
      ub[i] = MAX_VAL ;
      fprintf( stderr, "*** Adjusted upper bound: %d ***\n", ub[i] ) ;
      j++ ;
    }
    else{
  ...
  if( !valid_int(argv[j]) ){
    ...
    update_t.tv_sec = 1 ;
    update_t.tv_nsec = 0 ;
    fprintf( stderr, "*** Adjusted update delay: %ld ms***\n", update_t.tv_sec * 1000 ) ;
    j++ ;
  }
  else{
  ...
  if( !valid_int(argv[j]) ){
    ...
    read_t.tv_sec = 1 ;
    read_t.tv_nsec = 0 ;
    fprintf( stderr, "*** Adjusted I2C read delay: %ld ms***\n", read_t.tv_sec * 1000 ) ;
  }
  else{
  ...
  while( true ){
    ...
    num_bytes = LSM303_BYTES * NUM_303_VALS * NUM_303 ;
    memset( buffer, '\0', sizeof(char) * MAX_CHAR ) ;
    if( !i2c_read(I2C_FILE, buffer, num_bytes, ATMEGA_ADDR, &fd, open_file, close_file, oflags, mode) ){
      /* I2C bus read error. */
      strcpy( status, "disconnected" ) ;
    ...
    for( i = 0 ; i < NUM_303 ; i++ ){
      ...
      for( j = 0 ; j < (NUM_303_VALS / 2) ; j++ ){
        right_303_accel[(i*(NUM_303_VALS/2))+j] = (double)((signed short)((buffer[(i*(num_bytes/2))+(j*2)] << 8) | (buffer[(i*(num_bytes/2))+((j*2)+1)]))) ; 
      }
      for( ; j < NUM_303_VALS ; j++ ){
        right_303_mag[(i*(NUM_303_VALS/2))+(j%(NUM_303_VALS/2))] = 
         (double)((signed short)((buffer[(i*(num_bytes/2))+(j*2)] << 8) | (buffer[(i*(num_bytes/2))+((j*2)+1)]))) ;
      }
    }
    nanosleep( &read_t, &read_t_rem ) ;
